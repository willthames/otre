...
func TestCompleteSpan(t *testing.T) {
	...
	trace.addSpan(spans.Span{TraceID: "trace", ID: "root"})
	trace.addSpan(spans.Span{TraceID: "trace", ID: "child", ParentID: "root"})
	trace.addSpan(spans.Span{TraceID: "trace", ID: "grandchild1", ParentID: "child"})
	trace.addSpan(spans.Span{TraceID: "trace", ID: "grandchild2", ParentID: "child"})
...
func TestIsOlderThanAbsolute(t *testing.T) {
	...
	trace := new(Trace)
	trace.spans = make(map[SpanID]spans.Span)
	trace.addSpan(spans.Span{TraceID: "trace", Duration: 1000 * time.Millisecond, ID: "a", Timestamp: starttime})
	trace.addSpan(spans.Span{TraceID: "trace", Duration: 800 * time.Millisecond, ID: "b", Timestamp: starttime})
...
func TestTraceBuffer(t *testing.T) {
	...
	tbm = traceBuffer.AddSpan(spans.Span{TraceID: "trace", ID: "grandchild1", ParentID: "child"})
	if tbm.TraceDelta != 1 || tbm.SpanDelta != 1 {
		t.Errorf("Adding new trace with single span should cause span and trace deltas of 1")
	}
	tbm = traceBuffer.AddSpan(spans.Span{TraceID: "trace", ID: "root"})
	if tbm.TraceDelta != 0 || tbm.SpanDelta != 1 {
		t.Errorf("Adding single span to existing trace should cause span delta of 1")
	}
	traceBuffer.AddSpan(spans.Span{TraceID: "trace", ID: "child", ParentID: "root"})
	traceBuffer.AddSpan(spans.Span{TraceID: "trace", ID: "grandchild2", ParentID: "child"})
